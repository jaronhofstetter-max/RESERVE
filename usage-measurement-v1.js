(()=>{
  ...
  function mutate(fn){const metrics=read();fn(metrics);metrics.activated=!!(metrics.onboardingCompleted&&metrics.firstStockCreated);write(metrics);return metrics}
  function detectMilestones(){mutate(metrics=>{if(localStorage.getItem('reserveOnboardingDone')==='1')metrics.onboardingCompleted=true;try{const stock=JSON.parse(localStorage.getItem('reserveStock')||'[]');if(Array.isArray(stock)&&stock.length>0)metrics.firstStockCreated=true}catch(_){}})}
