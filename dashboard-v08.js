// TenderHawk dashboard. Uses only existing live SIMAP snapshot and current scoring.
(function(){
  let dashboardTop=[];
  ...
  function renderDashboard(){
    ...
    root.innerHTML=`
      ...
      <div class="th-panel th-matches"><h3>Top Matches für Ihr Unternehmen</h3>${top.map((a,i)=>{const x=a.x,o=a.o,s=o.score,cl=s>=75?'go':s>=55?'check':'stop';return `<div class="th-match"><div class="th-score ${cl}">${s}%</div><div><div class="th-match-title">${esc(x.title)}</div><div class="th-match-meta">${esc(x.buyer)} · ${esc(x.location||'Schweiz')} · ${deadlineDays(x)} Tage</div><div>${(o.reasons||[]).slice(0,3).map(r=>`<span class="th-chip">✓ ${esc(r)}</span>`).join(' ')}</div></div><div class="th-match-actions"><button onclick="openDashboardMatch(${i})">Bid/No-Bid</button>${x.url?`<a class="linkbtn" href="${esc(x.url)}" target="_blank" rel="noopener">SIMAP</a>`:''}</div></div>`}).join('')}</div>
      <div class="th-panel th-insight"><h3>TenderHawk Insight</h3><div class="th-insightbox"><b class="th-gold">${hot.length} GO-Kandidaten</b><br>von ${cache.length} aktuellen Projekten erreichen mindestens 75/100 im aktuellen Firmenprofil.<br><br>${urgent?`Bei ${urgent} Projekten liegt die Frist unter 14 Tagen.`:'Keine auffällig kurzen Fristen im aktuellen Bestand.'}</div><a class="th-btn" href="#" onclick="show('radar',document.querySelectorAll('.tab')[1]);return false">Alle Chancen ansehen</a></div>
  ...
  window.renderTenderDashboard=renderDashboard;
  addShell();
