/* RESERVE Smart Priorities v1.1 — expiry + demand + menu + replenishment decisions. */
(function(){
 const DAY=86400000;
 const stockRows=()=>{try{return Array.isArray(stock)?stock:[]}catch{return[]}};
 const recipeRows=()=>{try{if(typeof recipes!=='undefined'&&Array.isArray(recipes))return recipes}catch{}try{return Array.isArray(window.recipes)?window.recipes:[]}catch{return[]}};
 ...
 function recipePriorities(limit=5){const urgent=eatFirst(12),rows=recipeRows();return rows.map(r=>{const hits=(r.ingredients||[]).map(i=>urgent.find(x=>match(x.row.n,i.name))).filter(Boolean);if(!hits.length)return null;return{recipe:r,hits,score:hits.reduce((n,x)=>n+x.score,0)}}).filter(Boolean).sort((a,b)=>b.score-a.score).slice(0,limit)}
 ...
 window.RESERVE_SMART_PRIORITIES={version:'1.1',eatFirst,recipePriorities,buyNext,snapshot};
})();
