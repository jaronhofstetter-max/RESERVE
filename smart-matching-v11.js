...
(function(){
  ...
  const CAP_CLUSTERS=[
    ...
    {label:'BIM/CAD',terms:['bim','cad','dwg']},
    {label:'Revisionsunterlagen',terms:['revision','revisionsunterlagen','as-built','as built']},
    {label:'Inbetriebsetzung',terms:['inbetriebsetzung','inbetriebnahme','funktionsnachweis','funktionstest']},
    {label:'KBOB',terms:['kbob']},
    {label:'Minergie',terms:['minergie','eco']},
    {label:'Betrieb laufend',terms:['laufenden betrieb','laufender betrieb','betrieb aufrechterhalten']}
  ];

  const MUST_SIGNALS=[
    ['zwingend','Muss-Kriterium'],['muss ','Muss-Kriterium'],['nachweis','Nachweis erforderlich'],['referenz','Referenzanforderung'],
    ['zertifikat','Zertifikat'],['eignung','Eignungsprüfung'],['schluesselperson','Schlüsselperson'],['schlüsselperson','Schlüsselperson'],
    ['poenale','Pönale'],['pönale','Pönale'],['garantie','Garantie'],['sicherheitsleistung','Sicherheitsleistung']
  ];

  function profile(){
    const tradeText=norm(trades.value);
    const capabilityText=norm((caps.value||'')+' '+(refs.value||''));
    const regionText=norm((region.value||'')+' '+(kanton.value||''));
    const activeTrades=TRADE_CLUSTERS.filter(c=>containsAny(tradeText,c.terms));
    return {tradeText,capabilityText,regionText,activeTrades,maxValue:+maxv.value||0,capacity:(capacity.value||'mittel').toLowerCase()};
  }
  ...
  function regionMatch(p,x){
    ...
    if(!p.regionText.trim()) return false;
    const regionTerms=uniq(words(p.regionText));
    const aliases={be:['bern'],zh:['zurich','zuerich','zürich'],bs:['basel'],bl:['basel','baselland'],lu:['luzern'],sg:['st gallen','st. gallen'],ag:['aargau'],so:['solothurn']};
    return regionTerms.some(t=>loc.includes(t)||(aliases[t]||[]).some(a=>loc.includes(norm(a))));
  }
  ...
  window.opportunityScore=function(x){
    ...
    if(risks.includes('Sicherheitsleistung')) score-=2;

    score=Math.max(0,Math.min(100,Math.round(score)));
    const decision=score>=75?'GO-Kandidat':score>=55?'PRÜFEN':'NO-BID / geringe Passung';
    return {score,reasons:uniq(reasons),risks:uniq(risks),criteria:uniq(criteria),tradeHits:matchedTrades.map(c=>c.label),decision};
  };
  ...
  window.choose=function(i){
    ...
    try{
      const x=window._arr[i],o=opportunityScore(x);
      setTimeout(()=>{
        if(!analysisResult) return;
        const cl=o.score>=75?'g':o.score>=55?'w':'r';
        analysisResult.innerHTML=`<h2 class="${cl}">${o.score}/100 · ${esc(o.decision)}</h2><p><b>Warum passend:</b> ${esc(o.reasons.join(' · ')||'Keine starken positiven Signale erkannt.')}</p><p><b>Zu prüfen:</b> ${esc(o.risks.join(' · ')||'Keine offensichtlichen Warnsignale erkannt.')}</p><p><b>Erkannte Nachweise:</b> ${esc(o.criteria.join(', ')||'Keine explizit abgeglichenen Nachweise.')}</p><button onclick="analyze()">Detailanalyse neu berechnen</button>`;
      },0);
    }catch(e){}
  };

  if(typeof cache!=='undefined'&&cache.length) live();
})();
