...
async function compactImage(file){
  try{const bmp=await createImageBitmap(file),max=1100,s=Math.min(1,max/Math.max(bmp.width,bmp.height)),w=Math.max(1,Math.round(bmp.width*s)),h=Math.max(1,Math.round(bmp.height*s)),cv=document.createElement('canvas');cv.width=w;cv.height=h;cv.getContext('2d').drawImage(bmp,0,0,w,h);bmp.close?.();const blob=await new Promise(ok=>cv.toBlob(ok,'image/jpeg',0.78));return blob||file}catch{return file}
}
