/* RESERVE barcode search v2.3 — instant verified Swiss resolution plus OFF v3 enrichment. */
(function(){
  'use strict';
  ...
  function show(code,hit){
    code=digits(code);const input=$('barcodeInput'),name=$('scanName'),qty=$('scanQty'),expiry=$('scanExpiry'),cat=$('scanCat'),status=$('barcodeStatus'),box=$('barcodeResult');
  ...
  function resolveVerified(code){const hit=swiss(code);return hit?show(code,hit):null}
  ...
  function install(){
    ...
    const wrapped=async function(raw){const code=digits(raw),my=++sequence;if(!code)return original.apply(this,arguments);const local=resolveVerified(code);if(local)return local;const out=await original.apply(this,arguments);if(my!==sequence||!isUnknown(code))return out;const hit=await search(code);if(my!==sequence||!hit)return out;return show(code,hit)};
    wrapped.__reserveSearchV2=true;wrapped.__reserveSearchV2Original=original;api.lookup=wrapped;
    window.RESERVE_BARCODE_SEARCH={version:'2.3',lookup:search,lookupV3:search,isUnknown,variants,swissCatalog:SWISS,swiss,resolveVerified};
  }
