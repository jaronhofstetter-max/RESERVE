/* RESERVE expiry/consumption intelligence v1.0 */
(function(){
  ...
  function sorted(rows=window.stock||[],now=new Date()){return rows.map((row,index)=>({row,index,expiry:expiryState(row,now)})).sort((a,b)=>priority(a.row,now)-priority(b.row,now)||(a.expiry.days??99999)-(b.expiry.days??99999))}
  ...
  function consume(index,amount,reason='manual'){const row=(window.stock||[])[index],have=row&&window.parseAmount?.(row.q);amount=Number(amount);if(!row||!have||!Number.isFinite(amount)||amount<=0||amount>have.v)return{ok:false};const remaining=Math.max(0,have.v-amount);record(row,amount,have.u,reason);if(remaining<=0)window.stock.splice(index,1);else row.q=window.fmt(remaining,have.u);localStorage.setItem('reserveStock',JSON.stringify(window.stock));window.refresh?.();return{ok:true,remaining,unit:have.u}}
  function expiringWithin(days=7,rows=window.stock||[],now=new Date()){return sorted(rows,now).filter(x=>x.expiry.days!=null&&x.expiry.days>=0&&x.expiry.days<=days)}
  window.RESERVE_EXPIRY={version:'1.0',expiryState,sorted,expiringWithin,consume,history,record};
})();