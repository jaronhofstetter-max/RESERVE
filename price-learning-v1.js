/* RESERVE Price Learning v1.2 — learn unit prices from captured purchases. */
(function(){
 ...
 function learnedRate(name,u){const info=unitInfo(u),xs=matching(name,u);if(!xs.length)return null;const recent=xs.slice(-5),weight=recent.reduce((s,x)=>s+x.quantity,0);if(weight<=0)return null;const baseRate=recent.reduce((s,x)=>s+x.rate*x.quantity,0)/weight;return baseRate*info.factor}
 ...
 window.RESERVE_PRICE_LEARNING={version:'1.2',observations,learnedRate,estimate};
})();
