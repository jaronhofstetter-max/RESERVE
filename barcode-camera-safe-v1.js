/* RESERVE barcode camera safe v1.2 — low-load camera scanning plus preview-only diagnostic mode. */
(function(){
  'use strict';
  ...
  const previewOnly=params.get('reserveCameraOnly')==='1';
  ...
  async function previewStart(){
    backend='preview';
    stream=await navigator.mediaDevices.getUserMedia({video:{facingMode:{ideal:'environment'},width:{ideal:640,max:960},height:{ideal:480,max:720},frameRate:{ideal:12,max:15}},audio:false});
    const v=video();v.srcObject=stream;v.style.display='block';await v.play();
    status('Kamera-Vorschau aktiv – Barcode-Erkennung ist für diesen Test ausgeschaltet.','good');emit('started',{previewOnly:true});
  }
  ...
  async function safeStart(){
    ...
    safeStop();running=true;setButtons(true);status(previewOnly?'Kamera-Vorschau wird gestartet…':'Kamera wird gestartet…','');emit('start-request',{previewOnly});
    try{if(previewOnly)await previewStart();else if('BarcodeDetector'in window)await nativeStart();else await zxingStart()}catch(e){safeStop();status('Kamera konnte nicht gestartet werden. Bitte Kameraberechtigung prüfen oder erneut starten.','bad');emit('error',{name:e?.name||'Error'})}
  }
  function install(){
    ...
    window.addEventListener('pagehide',safeStop,{once:true});emit('installed',{previewOnly});
  }
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',install,{once:true});else install();
  window.RESERVE_CAMERA_SAFE={version:'1.2',previewOnly,start:safeStart,stop:safeStop,get state(){return{running,backend,previewOnly}}};
})();
