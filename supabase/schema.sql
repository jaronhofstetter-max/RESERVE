...
create table if not exists public.company_profiles (
  ...
  max_contract_value numeric not null default 0,
  capacity text not null default 'mittel' check (capacity in ('klein','mittel','gross')),
  match_threshold integer not null default 75 check (match_threshold between 0 and 100),
...
create table if not exists public.push_subscriptions (
  ...
  threshold integer not null default 75 check (threshold between 0 and 100),
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
