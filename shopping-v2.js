/* RESERVE smart shopping list v2.2: aggregate equal ingredients and compatible units without losing workflow metadata. */
(function(){
  ...
  function mergeMeta(target,item){
    ...
    if(item?.rotationCreatedAt){
      const t=Date.parse(item.rotationCreatedAt),current=Date.parse(target.rotationCreatedAt||'');
      if(!Number.isFinite(current)||Number.isFinite(t)&&t<current)target.rotationCreatedAt=item.rotationCreatedAt;
    }
  ...
  function mergeShopping(items){
    ...
    (items||[]).forEach(item=>{
      ...
      const unit=canonicalUnit(p.u),key=N(item.n)+'|'+unit+'|'+(item.reserveRotationGenerated?'reserve-rotation':'normal');
  ...
  window.RESERVE_SHOPPING={version:'2.2',mergeShopping};
