...
try{
  ...
  await page.waitForFunction(()=>window.reserveUsageMeasurementV1?.version==='2.1');
  ...
  await page.evaluate(()=>{
    ...
    const recipes=document.createElement('button');recipes.id='usage-test-recipes';recipes.textContent='Rezepte';document.body.appendChild(recipes);recipes.click();
    const scan=document.createElement('button');scan.id='usage-test-scan';scan.setAttribute('aria-label','Barcode scannen');document.body.appendChild(scan);scan.click();
    const add=document.createElement('button');add.id='scanAdd';add.textContent='Produkt hinzufügen';document.body.appendChild(add);add.click();
  ...
  console.log('Usage measurement v2.1 deferred metrics E2E passed');
