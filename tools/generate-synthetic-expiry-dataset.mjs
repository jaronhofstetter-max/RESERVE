...
function display(x){const yy=String(x.y).slice(-2),dd=String(x.d).padStart(2,'0'),mm=String(x.m).padStart(2,'0'),full=[`${dd}.${mm}.${x.y}`,`${dd}/${mm}/${yy}`,`${x.y}-${mm}-${dd}`],month=[`${mm}.${x.y}`,`${mm}/${yy}`],monthOnly=rnd()<.4,text=pick(monthOnly?month:full),lastDay=new Date(Date.UTC(x.y,x.m,0)).getUTCDate();return{text,target:monthOnly?`${x.y}-${mm}-${String(lastDay).padStart(2,'0')}`:x.iso,precision:monthOnly?'month_end':'exact_day'}}
function draw(text,mode){const p=Buffer.alloc(W*H),base=mode==='dark'?35:220,ink=mode==='dark'?210:35;for(let i=0;i<p.length;i++){const x=i%W,curve=Math.abs(x-W/2)/W,glare=mode==='glare'&&x>330&&x<410?45:0;p[i]=Math.max(0,Math.min(255,base+Math.floor((rnd()-.5)*24)+Math.floor(curve*20)+glare))}const scale=pick([5,6,7,8]),dot=rnd()<.55,gap=dot?1:0;let width=0;for(const c of text)width+=(font[c]?.[0].length||3)*scale+scale;let ox=Math.max(12,Math.floor((W-width)/2)+Math.floor((rnd()-.5)*50)),oy=70+Math.floor((rnd()-.5)*38),start=ox;for(const c of text){const g=font[c]||font[' '];for(let gy=0;gy<g.length;gy++)for(let gx=0;gx<g[gy].length;gx++)if(g[gy][gx]==='1')for(let py=gap;py<scale-gap;py++)for(let px=gap;px<scale-gap;px++){const xx=ox+gx*scale+px+Math.floor((gy-2)*(rnd()-.5)*.8),yy=oy+gy*scale+py;if(xx>=0&&xx<W&&yy>=0&&yy<H)p[yy*W+xx]=ink+Math.floor((rnd()-.5)*24)}ox+=(g[0].length+1)*scale}return{pixels:p,box:[Math.max(0,start-2),Math.max(0,H-(oy+5*scale)-2),Math.min(W,ox+2),Math.min(H,H-oy+2)]}}
...
for(let i=0;i<count;i++){const date=dateFor(i),shown=display(date),prefix=pick(['MHD ','BBE ','EXP ','']),mode=pick(modes),text=prefix+shown.text,file=`synthetic-${String(i+1).padStart(6,'0')}.png`,stem=path.join(out,'images',file.replace(/\.png$/,'')),rendered=draw(text,mode),box=rendered.box.join(' ');fs.writeFileSync(path.join(out,'images',file),png(rendered.pixels));fs.writeFileSync(stem+'.gt.txt',text+'\n');fs.writeFileSync(stem+'.box',[...[...text].map(c=>`${c} ${box} 0`),`\t ${box} 0`].join('\n')+'\n');rows.push({image:`images/${file}`,date:shown.target,precision:shown.precision,text,synthetic:true,split:'train',surface:mode,seed,index:i})}
