...
const fields=['description','equipment','prepNotes','doneness','substitutions','leftovers','safety','resilience'];
...
const incomplete=rows.filter(r=>r.status==='approved'&&(r.missing.length||r.shortSteps));
const counts=Object.fromEntries(fields.map(k=>[k,incomplete.filter(r=>r.missing.includes(k)).length]));
console.log(`RESERVE Qualitätsaudit: ${recipes.length} Rezepte, ${incomplete.length} approved mit Ausbaupotenzial.`);
console.log('Fehlende Detailfelder:',counts);
console.log(`Rezepte mit knappen Kochschritten: ${incomplete.filter(r=>r.shortSteps).length}`);
if(process.env.REQUIRE_COMPLETE_RECIPE_QUALITY==='1'&&incomplete.length){
 console.error('\nUnvollständige approved Rezepte:');
 incomplete.forEach(r=>console.error(` - ${r.id}: ${r.missing.join(', ')||'nur Kochschritte'}${r.shortSteps?`; ${r.shortSteps} knappe Schritte`:''}`));
 process.exit(1);
}
