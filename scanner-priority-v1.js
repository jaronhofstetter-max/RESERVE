/* RESERVE scanner priority v1.0 — pause nonessential rendering while camera/scan work is active on mobile. */
(function(){
  'use strict';
  ...
  function wrap(name){
    ...
    if(!originals.has(name))originals.set(name,fn);
  ...
  setInterval(()=>{
    if(!busy())document.documentElement.classList.remove('reserve-scanner-busy');
    install();
  },500);
  ...
  window.RESERVE_SCANNER_PRIORITY={version:'1.0',busy,install,get active(){return active},get holdUntil(){return holdUntil}};
})();
